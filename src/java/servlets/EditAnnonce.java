        ...
        try {
            ...
            if(membre.getId()==0)
                ...
                if(request.getParameter("idAnnonce")==null)
                    ...
                    Annonce annonce=new Annonce(membre, idAnnonce);
                    if(annonce.getMembre().getId()==0)
                        request.setAttribute("info", 1);
                    else {
                    annonce.initInfosEdit();
                    if(annonce.getId()==0)
                        request.setAttribute("info", 2);
                    else {
                        if(request.getParameter("kermit")!=null) {
                            ...
                            annonce.enregAnnonceEdit(request);
                            if(annonce.getTest()==1)
                                response.sendRedirect("./edit-annonce-photo.html");
                        }
            ...
            RequestDispatcher dispatch = getServletContext().getRequestDispatcher("/WEB-INF/scripts/editannonce.jsp");
