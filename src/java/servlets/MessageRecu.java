        ...
        try {
            ...
            if(membre.getId()==0)
                ...
                if(request.getParameter("idMessage")==null)
                    ...
                    message.initRecu(membre);
                    //out.println(message.getErrorMsg());
                    if(message.getId()==0||message.getMembreDestinataire().getId()==0||message.getMembreExpediteur().getId()==0)
                        request.setAttribute("info", 2);
                    else {
                        if(request.getParameter("kermit")!=null) {
                            message.getPostsContact(request);
                            message.verifPostsContact(request);
                            message.enregContact2(request);
                        }
                        request.setAttribute("message", message);
                    }
                }
            }
            RequestDispatcher dispatch = getServletContext().getRequestDispatcher("/WEB-INF/scripts/messagerecu.jsp");
